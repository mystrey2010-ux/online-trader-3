...
# ============================================
    ...
    def _generate_and_apply_hypotheses(self, current_regime="NEUTRAL", avg_ret=None, max_dd=None, sharpe=None):
        ...
        regime_tuning_map = {
            ...
            "BEAR": {  # Bear market: prices crashing, high risk
                ...
                "rsi_period": {"direction": "longer", "new_value_multiplier": 1.33, "reasoning": "Smoother RSI in choppy bear market"}  # D-062
            },
    ...
    def run_cycle(self):
        ...
        try:
            ohlcv = self.exchange.fetch_ohlcv(symbol, timeframe='1m', limit=50)
            df = pd.DataFrame(ohlcv, columns=['ts', 'o', 'h', 'l', 'c', 'v'])
            prices = df['c'].values
